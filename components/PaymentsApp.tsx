...
function mockAddressFor(net: string) {
  const m: any = { TRC20: "TQ5E...9XZ1", BEP20: "0x5ab3...F29c", ERC20: "0x91D0...A77e" };
...
/* ----------- Bonus panel left as-is (no changes) ----------- */
/* ... (оставляю без изменений — твоя текущая логика бонусов) ... */
//
// Чтобы ответ был короче, блок BonusActivationPanel не менялся.
// Скопируй его из своего файла как есть.
//
...
function DepositBitcart() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // пытаться забрать адрес, если ещё пусто
        if (!invoice.address) {
          const addr =
            data?.address ||
            data?.addresses?.[0] ||
  ...
  }, [invoice?.id, status, bonusAlreadyApplied, invoice?.amount, invoice?.address]);
  ...
  const createInvoice = async () => {
    ...
    try {
      ...
      // сразу подтянем адрес, если его нет в ответе
      try {
        const r2 = await fetch(`/api/invoices/${inv.id}`);
        if (r2.ok) {
  ...
  // Показать реквизиты/QR в модалке Bitcart
  const openDetails = () => {
  ...
  // Проверить оплату — открыть checkout (public_url)
  const openCheckout = () => {
    if (invoice?.payUrl) {
      window.open(invoice.payUrl, "_blank", "noopener,noreferrer");
      return;
    }
    // на крайний случай — тоже модалка
    openDetails();
  };
            ...
            <>
              ...
              <div className="rounded-2xl p-4 bg-black/40 border border-[#2a2f3a] flex gap-3">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <KVP label="Адрес" value={invoice.address ?? "— появится в модалке"} copyable={!!invoice.address} />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <Button onClick={openDetails} className="rounded-2xl bg-[#F5A623] text-black hover:bg-[#ffb739]">Открыть оплату</Button>
                <Button onClick={cancel} variant="secondary" className="rounded-2xl">Отменить</Button>
                <Button onClick={openCheckout} className="rounded-2xl col-span-2 bg-emerald-600 hover:bg-emerald-700">
