...
const ENV = {
  BITCART_ADMIN_URL: (typeof process !== 'undefined' && (process as any)?.env?.NEXT_PUBLIC_BITCART_ADMIN_URL) || 'https://pay.bybitpay.pro',
};
...
function load<T = any>(key: string, def: T): T { try { const v = localStorage.getItem(key); return v ? JSON.parse(v) as T : def; } catch { return def; } }

function readDB() {
  try { return JSON.parse(localStorage.getItem("byvc.db") ?? "{\"users\":{}}") as { users: Record<string, any> }; }
  catch { return { users: {} }; }
}
function writeDB(db: any) { try { localStorage.setItem("byvc.db", JSON.stringify(db)); } catch {} }
function guessUsername() {
  try { const tg = (window as any)?.Telegram?.WebApp?.initDataUnsafe?.user?.username; if (tg) return tg; } catch {}
  try { const k = localStorage.getItem('byvc.currentUser') || localStorage.getItem('byvc.lastUser'); if (k) return k; } catch {}
  return 'guest';
}

function readInvoices() { try { return JSON.parse(localStorage.getItem('byvc.api.invoices') ?? '[]') as any[]; } catch { return []; } }
function writeInvoices(arr: any[]) { try { localStorage.setItem('byvc.api.invoices', JSON.stringify(arr)); } catch {} }

function mockAddressFor(net: string) {
  const m: any = { TRC20: "TQ5E...9XZ1", BEP20: "0x5ab3...F29c", ERC20: "0x91D0...A77e", SOL: "9Xg7...JkT2" };
  return m[net] || "0x000...000";
}

const api = {
  async createDeposit(payload: { store_id: string; price: number; currency: string; username: string; network: string; metadata?: any }) {
    const { store_id, price, currency, username, network, metadata } = payload;
    await new Promise(r => setTimeout(r, 300));
    const inv = {
      id: `inv_${Date.now()}`,
      store_id, price, currency, username, network,
    ...
    const all = readInvoices(); all.unshift(inv); writeInvoices(all);
    return inv;
  },
  async webhookBitcartPaid(invoiceId: string) {
    ...
    const urec = copy.users[username] || { balance: 0, txs: [], referrals: { earned: 0, count: 0, rate: 50 }, cardActive: false, bonuses: {} };
    ...
    urec.txs = [
      { id: `top_${Date.now()}`, type: 'topup', amount: a, ccy: inv.currency || 'USDT', network: inv.network, ts: new Date().toISOString(), status: 'Confirmed', invoiceId },
      ...(bonusAmt > 0 ? [{ id: `bonus_${Date.now()}`, type: 'bonus', amount: bonusAmt, ccy: inv.currency || 'USDT', network: inv.network, ts: new Date().toISOString(), status: 'Applied', reason: tier === 200 ? 'First deposit ≥500' : 'First deposit ≥100' }] : []),
      ...(urec.txs || []),
    ];
    if (qualifies) urec.bonuses.firstBonusApplied = true;
    copy.users[username] = urec; writeDB(copy);
    return { ...inv, appliedBonus: bonusAmt, tier };
  },
  async getInvoice(id: string) { const inv = readInvoices().find((x: any) => x.id === id); return inv || null; },
};
    ...
    <div className="min-h-screen w-full bg-[#0f1115] text-neutral-100 p-4">
      <header className="max-w-md mx-auto flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-2xl bg-[#F5A623]/15 grid place-items-center"><Wallet className="h-5 w-5 text-[#F5A623]" /></div>
          <div>
...
function BonusActivationPanel({ amount, isFirst }: { amount: number; isFirst: boolean }) {
  ...
  if (!isFirst) {
        ...
        <Card className="rounded-2xl bg-[#1b2029] border-[#2a2f3a]">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base flex items-center gap-2"><Info className="h-4 w-4 text-yellow-400" />Повторное пополнение</CardTitle>
    ...
    return (
      <Card className="rounded-2xl bg-[#1b2029] border-[#2a2f3a]">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2 text-emerald-300"><ShieldCheck className="h-4 w-4" />Повторное пополнение</CardTitle>
            <Badge variant="secondary" className="rounded-full">Бонус уже использован</Badge>
          </div>
  ...
  if (lt100) {
    const ex = 100; const exBonus = ex; const exTotal = ex + exBonus;
    return (
      <Card className="rounded-2xl bg-[#1b2029] border-[#2a2f3a]">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2"><Info className="h-4 w-4 text-yellow-400" />Бонусы и активация</CardTitle>
            <Badge variant="secondary" className="rounded-full">Бонус доступен</Badge>
          </div>
          <CardDescription className="text-neutral-400">Пополнение менее 100 USDT — без бонусов и активации. Бонус сохранится до первого пополнения ≥100 USDT.</CardDescription>
        </CardHeader>
        <CardContent className="text-sm space-y-3 text-neutral-200">
          ...
          <div className="flex items-start gap-2"><Info className="h-4 w-4 text-neutral-400 mt-0.5" />Если бонус начислен на сумме 100–499, то возможность +200% сгорает.</div>
          <div className="rounded-xl bg-black/30 p-3 text-xs text-neutral-300">Например: пополнив на <span className="text-white font-semibold">{ex} USDT</span>, вы получили бы бонус <span className="text-[#F5A623] font-semibold">+{exBonus} USDT</span> и на карте было бы <span className="text-white font-semibold">{exTotal} USDT</span>.</div>
          <Calc />
        </CardContent>
  ...
  if (gte500) {
    return (
      <Card className="rounded-2xl bg-[#F5A623]/15 border border-[#F5A623]/40">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2 text-[#F5A623]"><Gift className="h-4 w-4" />Максимальный бонус к первому пополнению</CardTitle>
        ...
        <CardContent className="text-sm space-y-3 text-neutral-100">
          ...
          <Calc />
          <div className="text-xs text-neutral-300">Возможность +200% действует только если первое квалифицирующее пополнение ≥500 USDT.</div>
        </CardContent>
  ...
  return (
    <Card className="rounded-2xl bg-[#1b2029] border-[#2a2f3a]">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base flex items-center gap-2 text-emerald-300"><ShieldCheck className="h-4 w-4" />Бонусы будут начислены</CardTitle>
          <Badge variant="secondary" className="rounded-full">Бонус доступен</Badge>
        </div>
        <CardDescription className="text-neutral-400">Первое пополнение от 100 до 499.99 USDT даёт +100%.</CardDescription>
      </CardHeader>
      <CardContent className="text-sm space-y-3 text-neutral-200">
        <div className="flex items-start gap-2"><Gift className="h-4 w-4 text-emerald-300 mt-0.5" />Бонус к первому пополнению: <span className="font-semibold">+100%</span>.</div>
        ...
        <Calc />
        <div className="text-xs text-neutral-300">Если первое квалифицирующее пополнение <span className="font-semibold text-white">&lt; 500 USDT</span>, возможность получить +200% сгорает.</div>
      </CardContent>
...
function DepositBitcart() {
  ...
  const [success, setSuccess] = useState<null | { amount: number; bonus: number; total: number; balance?: number }>(null);
  ...
  const createInvoice = async () => {
    setCreating(true);
    const payload = { price: Number(amount || 0), currency: 'USDT', username, network, metadata: { username, network, firstDepositEligible: isFirstEligible } } as any;
    try {
      const res = await fetch('/api/payments/deposits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (!res.ok) throw new Error('Failed to create invoice');
      const inv = await res.json();
      setInvoice({ id: inv.id, amount: inv.price ?? inv.amount, ccy: inv.currency, network: inv.network ?? network, address: inv.address, payUrl: inv.payUrl });
      setStatus('pending');
      ...
      return;
    } catch (e) { console.warn('Falling back to mock invoice (dev):', e); }
    const inv = await api.createDeposit({ store_id: 'STORE_DEMO_1', price: payload.price, currency: 'USDT', username, network, metadata: payload.metadata });
 ...
 const openModal = () => {
  ...
  const cancel = () => { setStatus("cancelled"); setInvoice(null); setExpiresAt(0); };
  ...
  return (
    <div className="space-y-4 relative">
          ...
          {!invoice && (
            <>
              <div className="grid grid-cols-4 gap-2">
                {networks.map(n => (
                  <Button key={n.code} onClick={() => setNetwork(n.code)} className={`${network === n.code ? 'bg-[#F5A623] text-black hover:bg-[#ffb739]' : 'bg-black/40 text-neutral-200 hover:bg-black/60'} rounded-xl`}>{n.code}</Button>
                ))}
              </div>
              <Row label="Сеть" value={network} hint={`Комиссия ~${networks.find(n => n.code === network)?.fee} USDT • ${networks.find(n => n.code === network)?.eta}`} />
              <div className="grid gap-2">
                <Label>Сумма (USDT)</Label>
                <UInput type="number" value={amount} onChange={e => setAmount(Number(e.target.value))} />
              </div>
              <Button disabled={creating || amount <= 0} onClick={createInvoice} className="w-full rounded-2xl h-12 bg-emerald-600 hover:bg-emerald-700">
                {creating ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Создание счёта...</>) : (<>Пополнить</>)}
              </Button>
            </>
          )}
          ...
          {invoice && (
            <>
              <div className="rounded-2xl p-3 bg-black/40 border border-[#2a2f3a] flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-xs text-neutral-300"><Clock className="h-4 w-4" /> {pad(Math.floor(secs / 60))}:{pad(secs % 60)}</div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button onClick={openModal} className="rounded-2xl bg-[#F5A623] text-black hover:bg-[#ffb739]">Открыть оплату</Button>
                <Button onClick={cancel} variant="secondary" className="rounded-2xl">Отменить</Button>
                <Button onClick={markPaid} className="rounded-2xl col-span-2 bg-emerald-600 hover:bg-emerald-700"><Check className="h-4 w-4 mr-2" />Отметить как оплачено (мок)</Button>
              </div>
            </>
          )}
        </CardContent>
      ...
      {success && (
        <SuccessModal open={!!success} onClose={() => setSuccess(null)} amount={success.amount} bonus={success.bonus} total={success.total} balance={success.balance} />
      )}
    </div>
  );
}
...
function SuccessModal({ open, onClose, amount, bonus, total, balance }: { open: boolean; onClose: () => void; amount: number; bonus: number; total: number; balance?: number; }) {
  if (!open) return null;
  return (
