...
/** базовый хост витрины (без /admin) — нужен для прямой ссылки на инвойс */
...
function Deposit() {
  ...
  const [network] = useState<string>("TRC20"); // зафиксировано
    ...
    const poll = async () => {
      try {
        ...
        if (st.includes("confirmed") || st.includes("paid")) {
          setStatus("confirmed");
          // очищаем UI к исходному состоянию
          setTimeout(() => {
            setInvoice(null);
            setStatus("idle");
            setExpiresAt(0);
  ...
  const createInvoice = async () => {
    ...
    try {
      ...
      const ttl = inv?.expiresAt
        ...
        : Date.now() + 15 * 60 * 1000; // безопасный дефолт
  ...
  return (
    <div className="space-y-4">
      ...
      {/* инфо-блок про бонус/активацию — оставил лаконично */}
